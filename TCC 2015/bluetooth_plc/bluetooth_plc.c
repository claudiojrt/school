...
#define		END		10																					/// ENDEREÇO DO DISPOSITIVO
...
char	dado_rx0=0;																						//Dado de recepção da comunicação serial
...
void protocol(){																						/// PROTOCOLO
	if((dado_rx&0b00001100)>3){
		acao();
	}
	else if(((dado_rx&0b11110000)>>4)==END){
		acao();
	}
}
...
void set_bit2(){																						/// SETA BITS EM dado_rx
	byte_cont++;																						//Proximo bit
...
void test_bit(){																						/// TESTE DE BIT (ENVIO)
...
void interrupt interrupts(void){																		/// INTERRUPÇÕES
	...
	if(RCIF==1 && RCIE==1){																				/// RECEPÇÃO SERIAL
		if(FERR==1){																					//Se frame error
			lixo=RCREG;																					//Lixo
		}
		else if(OERR==1){																				//Se erro de overrun
			CREN=0;																						//Reseta CREN
			CREN=1;																						// "
		}
		else{																							//Se dado
			dado_rx0=RCREG;																				//Guarda em dado_rx0
			/// TODO
			/// TESTE DOS DADOS
		}
		RCIF=0;																							//Zera flag recepção serial
	}
}
...
void serial_init(void){																					/// INICIALIZA SERIAL
	SYNC=0;																								// "
	BRGH=1;																								// "
	BRG16=0;																							// "
	SPBRGH=0;																							// "
